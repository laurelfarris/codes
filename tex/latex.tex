...
\section{Line spacing and indentation}
...
\textcolor{red}{Need to move coloring for sections here. Organize
things by the item itself (sections, lists, etc.),
not by what is being done (coloring, sizing, etc.)}
\paragraph{Change font size, make font bold, etc.}
\begin{verbatim}
\usepackage{titlesec}
\titleformat*{\section}{\LARGE\bfseries}
\end{verbatim}
Do this with any of the nested sections.
...
You will have to run {\tt pdflatex} twice.
...
\begin{description}
    ...
    \item [listparindent]
    \item [labelwidth] Designated space for label. Good for lining up
        text when labels are left-aligned.
    \item [labelsep] Space between label and following text
\end{description}
Vertical spacing:
\begin{description}
    ...
    \item [partopsep] extra space added to topsep when environment starts
        a new paragraph
    \item [parsep]
    \item [itemsep] Space between items
\end{description}

Apparently topsep is also bottomsep $\ldots$?
...
\end{verbatim}
This puts the labels in typewriter font. The hspace command does not appear
...
This may be required for a dissertation :(
\begin{verbatim}
...
In {\tt think\_python.tex}, add part for using straight single quotes
in verbatim environment.

What's the difference between \verb|\clearpage| and \verb|\newpage|?

What does the `*' do in the titleformat part of preamble?

How to condense the toc, make singlespaced, not bold, keep
multiple columns from overlapping, etc.
