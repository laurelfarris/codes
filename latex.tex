...
% 05 May 2020 -- packge to highlight text: \hl{text}
\usepackage{soul}
...
%\lfoot{\today} % current date on bottom left corner
...
\href{http://mirrors.ibiblio.org/CTAN/macros/latex/contrib/comment/comment.pdf}{
    Comment package}

Section 3.1 explains the \texttt{comment.cut} file created when running
pdflatex\ldots should contain the text commented in the main \texttt{.tex} file.

\begin{lstlisting}

\usepackage{comment}
\includecomment{mycomment} % Include content in output pdf
\excludecomment{mycomment} % Exclude content, i.e. treat as real comment.
...
\hypersetup{
    hidelinks,         % ?
    linktoc=all,       % ?
    linktocpage=false, % only page #s are clickable links in toc
    ...
    colorlinks=false,  % Allow color for link and anchor text...
    urlcolor=magenta   %   linked URLs
    linkcolor=red      %   normal internal links
    citecolor=green,   %   bibliographical citations
    anchorcolor=black, %   anchor (target) text
    filecolor=cyan,    %   links that open local files
}


% Make \href{} link text bold
\let\oldhref\href
\renewcommand{\href}[2]{\oldhref{#1}{\bfseries#2}}

% Change style of urls (same as body text, roman, sans serif, bold, etc.)
%  NOTE: url text is not the same as hyperlink text in \href{}.
    \urlstyle{same}
    \urlstyle{rm}
    \urlstyle{sf}
...
\href{https://www.overleaf.com/learn/latex/Hyperlinks#Reference_guide}{
    hypersetup options}
...
\begin{itemize}
    ...
    \item angle=90|180|etc\ldots  % rotate graphic counter-clockwise
\end{itemize}
...
To set a directory location for input files other than current (see
\href{https://vi.stackexchange.com/questions/84/how-can-i-copy-text-to-the-system-clipboard-from-vim}%
{this link}):
\begin{lstlisting}
\makeatletter
\def\input@path{
    {$HOME/Dropbox/Dissertation/Test/}
}
...
\begin{lstlisting}
% Print the numerical value of a latex length (in body text):
\usepackage{printlen}
% \begin{document}
% ...
% \printlength{length}
%   units specified by \uselengthunit{unit},
%   'unit' = pt, pc, in, mm, cm, bp, dd, cc
\end{lstlisting}



Define custom width in preamble if used many times in document, and you
don't want to manually change every single line if you decide to adjust
the width.
\begin{lstlisting}
\newlength{\mylength}
\setlength{\mylength}{0.8\textwidth}
\end{lstlisting}
